...
def run(cfg: PhysicsNeMoConfig) -> None:
    ...
    # ze = ZeroEquation(
    #     nu=nd_fluid_viscosity / nd_fluid_density, dim=3, time=False, max_distance=0.1
    # ) # zlength /2/ scale = 340/2/100 = 1.7
    ns = NavierStokes(nu=nd_fluid_viscosity / nd_fluid_density, rho=nd_fluid_density, dim=3, time=False)
    ...
    # add monitor
    # front pressure
    plane_param_ranges = {Symbol("x_pos"): 0.5}
    ...
    # back pressure
    plane_param_ranges = {Symbol("x_pos"): 1}
    invar_pressure = limerock.integral_plane.sample_boundary(
        5000,
        parameterization=plane_param_ranges,
    )
